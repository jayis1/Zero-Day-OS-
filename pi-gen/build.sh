#!/bin/bash -e
# pi-gen/build.sh
# Main build script for ZERO-DAY OS
# Based on the official Raspberry Pi pi-gen build system
...
EOF
		...
		if [ -f "${i}-packages-nr" ]; then
			...
			PACKAGES="$(sed -f "${SCRIPT_DIR}/remove-comments.sed" < "${i}-packages-nr" | tr '\n' ' ')"
			if [ -n "$PACKAGES" ]; then
				on_chroot << EOF
...
EOF
		...
		if [ -f "${i}-packages" ]; then
			...
			PACKAGES="$(sed -f "${SCRIPT_DIR}/remove-comments.sed" < "${i}-packages" | tr '\n' ' ')"
			if [ -n "$PACKAGES" ]; then
				on_chroot << EOF
...
# ── ZERO-DAY OS Configuration ──
export PI_GEN=${PI_GEN:-zeroday-pi-gen}
...
export TARGET_HOSTNAME=${TARGET_HOSTNAME:-zeroday}

export FIRST_USER_NAME=${FIRST_USER_NAME:-root}
export FIRST_USER_PASS=${FIRST_USER_PASS:-zeroday}
export DISABLE_FIRST_BOOT_USER_RENAME=${DISABLE_FIRST_BOOT_USER_RENAME:-1}
export PASSWORDLESS_SUDO="${PASSWORDLESS_SUDO:-1}"
export WPA_COUNTRY
export ENABLE_SSH="${ENABLE_SSH:-2}"
